...
async fn measure_1k_persisted_size() {
    ...
    let graph_sz = std::fs::metadata(&graph).map(|m| m.len()).unwrap_or(0);
    let data_sz = std::fs::metadata(&data).map(|m| m.len()).unwrap_or(0);
    let meta_sz = std::fs::metadata(&meta).map(|m| m.len()).unwrap_or(0);
    println!(
