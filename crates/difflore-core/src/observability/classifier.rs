...
fn determine_obs_type(input: &ClassifyInput<'_>) -> String {
    ...
    if let Some(diff) = input.diff {
        ...
        if diff_has_change_lines(diff) {
            return "change".to_owned();
        }
    }
    if let (Some(old), Some(new)) = (input.old_text, input.new_text)
...
fn diff_has_change_lines(diff: &str) -> bool {
    diff.lines().any(|line| {
        !line.starts_with("---")
            && !line.starts_with("+++")
...
/// `true` when the path matches a hardcoded secret-bearing filename pattern.
...
mod tests {
    ...
    #[test]
    fn classify_edit_whitespace_only_uses_text_fallback_when_diff_has_no_change_lines() {
        let old = "let x=1;let y=2;";
        let new = "let x = 1;\nlet y = 2;";
        let diff = "@@ -1 +1 @@\n";
        let inp = input("Edit", "src/foo.rs", Some(diff), Some(new), Some(old));
    ...
    #[test]
    fn privacy_guard_does_not_substring_match_normal_source_names() {
        for path in &[
            "src/credentials_provider.rs",
            "src/keybindings.ts",
            "src/pem_parser.rs",
            "src/monkey.rs",
        ] {
            assert!(
                !is_privacy_denied(path),
                "expected source path to be allowed: `{path}`"
            );
        }
    }
