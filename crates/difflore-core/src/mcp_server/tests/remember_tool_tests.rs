...
async fn tools_list_advertises_expected_tools() {
    ...
    sorted_names.sort();
    let mut allowed = ALLOWED_MCP_TOOL_NAMES
        .iter()
        .map(|tool| (*tool).to_owned())
        .collect::<Vec<_>>();
    allowed.sort();
    assert_eq!(
        sorted_names, allowed,
        "tools/list must match the enforced MCP allowlist"
    );
    for required in [
        "remember_rule",
        "search_rules",
    ...
    assert!(
        ...
        "get_rules schema should expose optional file path hint: {get_rules}"
    );
    assert_eq!(
    ...
    for read_only in [
        ...
        let tool = result["tools"]
            ...
            .unwrap()
            .iter()
            .find(|t| t["name"].as_str() == Some(read_only))
            .unwrap_or_else(|| panic!("{read_only} tool missing"));
        assert_eq!(
            tool["annotations"]["readOnlyHint"].as_bool(),
            Some(true),
            "{read_only} should advertise read-only annotation: {tool}"
        );
    }
    let autopilot_log = result["tools"]
        .as_array()
        .unwrap()
        .iter()
    ...
    assert!(
        ...
        "autopilot log tool should be read-only and CLI-directed: {autopilot_log_description}"
    );
    assert_eq!(
        autopilot_log["annotations"]["readOnlyHint"].as_bool(),
        Some(true),
        "autopilot log should advertise read-only annotation: {autopilot_log}"
    );
    assert!(
        autopilot_log["_meta"]["governance"]
            .as_str()
            .unwrap_or_default()
            .contains("read_only_for_ai"),
        "autopilot log should expose governance metadata: {autopilot_log}"
    );
}

#[tokio::test]
async fn memory_tools_read_user_remembered_active_rules() {
    let state = build_state().await;
    let remember = call_ok(
        ...
        &call_tool(
            ...
            "remember_rule",
            json!({
                "title": "Use typed route params",
                "body": "Parse route params through the typed helper before reaching business logic.",
    ...
    let rule_id = remember["_meta"]["rule_id"]
        ...
        .expect("rule id")
        .to_owned();
    let item_id = remember["_meta"]["item_id"]
        .as_str()
        .expect("item id")
        .to_owned();
    assert_eq!(remember["_meta"]["state"].as_str(), Some("active"));
    assert_eq!(
        remember["_meta"]["capture_status"].as_str(),
        Some("captured_active")
    );
    ...
    let listed = items
        .iter()
        .find(|item| item["itemId"].as_str() == Some(item_id.as_str()))
        .expect("remembered rule is listed");
    assert_eq!(listed["kind"].as_str(), Some("rule"));
    assert_eq!(listed["active"].as_bool(), Some(true));
    assert_eq!(listed["servedToAgents"].as_bool(), Some(true));
    ...
    assert!(
        detail_body["body"]
            .as_str()
            .unwrap()
            .contains("typed helper"),
        "detail should expose the active rule body: {detail_body}"
    );
    assert_eq!(
        crate::skills::rule_status(&state.db, &rule_id)
            .await
            .expect("status")
            .as_deref(),
        Some("active"),
        "explicit remember requests should activate rules immediately"
    );
}
...
async fn memory_activity_tool_labels_surface_proof_carefully() {
    ...
    assert!(
        ...
        "activity note must avoid overclaiming influence: {body}"
    );
}
...
async fn memory_autopilot_tools_return_structured_json_read_only() {
    ...
    assert!(
        ...
        "autopilot log should return structured event array: {log_body}"
    );
    assert!(
        log_body["note"]
            .as_str()
            .unwrap_or_default()
            .contains("CLI"),
        "autopilot log should direct mutations to CLI: {log_body}"
    );
    assert!(
        log_result["_meta"]["governance"]
            .as_str()
            .unwrap_or_default()
            .contains("read_only_for_ai"),
        "autopilot log response should carry read-only governance metadata: {log_result}"
    );
}

#[tokio::test]
async fn remember_rule_activates_user_requested_memory_then_recalls() {
    let state = build_state().await;
    ...
    assert_eq!(
        remember["_meta"]["requires_user_approval"].as_bool(),
        Some(false)
    );
    assert_eq!(remember["_meta"]["served_to_agents"].as_bool(), Some(true));
    ...
    assert!(
        body_text.contains("active") && body_text.contains("available to agents"),
        "confirm text explains immediate activation: {body_text}"
    );
    let status = crate::skills::rule_status(&state.db, &rule_id)
        .await
        .expect("status")
        .expect("rule status");
    ...
    assert!(
        ...
        "remembered active rule should be in search output, got: {results:?}"
    );
    ...
    assert!(
        ...
        "remembered active rule should be in get_rules output, got: {recall_text}"
    );
}
...
async fn remember_rule_self_managed_gitlab_real_remote_refresh_drives_scope() {
    ...
    let result = std::panic::AssertUnwindSafe(async {
        ...
        // End-to-end recall on the same self-managed scope. Explicit user
        // remember requests are active immediately.
        if let Some(index_pool) = state.index_pool.as_ref() {
            crate::context::orchestrator::ensure_rules_indexed_for_repo_scopes_with_embedding_timeout(
                &state.db,
                index_pool,
                &[EXPECTED_SCOPE.to_owned()],
                None,
            )
            .await
            .expect("index promoted self-managed rule");
        }
        let (search_body, _) = call_tool_json(
            &state,
            8,
...
fn run_git_in(path: &std::path::Path, args: &[&str]) {
    let output = crate::infra::git::git_command(path)
        .args(args)
        .output()
...
async fn retired_one_shot_rule_tool_is_not_callable() {
    ...
    assert!(
        message.contains("Unknown or unapproved MCP tool: get_relevant_rules")
            && message.contains("explicit allowlist"),
...
async fn remember_rule_dedup_returns_strengthened_meta() {
    ...
    assert_eq!(
        second["_meta"]["capture_status"].as_str(),
        Some("strengthened_active")
    );
    assert_eq!(
    ...
    assert_eq!(
        crate::skills::count_pending_candidates(&state.db, None)
            .await
            .expect("pending count"),
        0,
        "active remember_rule captures must not create pending drafts"
    );
    assert_eq!(
        crate::skills::rule_status(&state.db, &first_id)
            .await
            .expect("status")
            .as_deref(),
        Some("active"),
...
fn test_repo_scopes() -> Vec<String> {
    vec![TEST_REPO.to_owned()]
}
...
async fn promote_rule_for_test(state: &McpState, rule_id: &str) {
...
#[tokio::test]
async fn remember_rule_refreshes_mcp_index_with_local_profile() {
    let _guard = repo_detection_test_guard().await;
    ...
    let result = call_ok(
        ...
        &call_tool(
            ...
            "remember_rule",
            json!({
                "title": "Keep MCP remember indexing local",
                "body": "When a user asks to remember a rule, refresh the MCP index locally so the agent hot path never waits on remote query embeddings.",
                "file_patterns": ["src/**/*.rs"]
            }),
        ),
    )
    .await;
    let rule_id = result["_meta"]["rule_id"]
        .as_str()
        .expect("rule_id present");
    let index_pool = state.index_pool.as_ref().expect("test index pool");

    let profile: String =
        sqlx::query_scalar("SELECT value FROM rule_index_meta WHERE key = 'embedding_profile'")
            .fetch_one(index_pool)
            .await
            .expect("embedding profile meta");
    assert_eq!(
        profile,
        crate::context::embedding::local_embedding_profile(),
        "remember_rule must refresh the MCP index with local embeddings"
    );

    let chunk_count: i64 =
        sqlx::query_scalar("SELECT COUNT(*) FROM rule_chunks WHERE skill_id = ?1")
            .bind(rule_id)
            .fetch_one(index_pool)
            .await
            .expect("indexed remembered rule");
    assert_eq!(chunk_count, 1, "remembered active rule must be indexed");

    let diag = crate::context::gather_embedding_diagnostics(index_pool).await;
    assert!(
        !diag.degraded,
        "local MCP index should not be diagnosed as degraded: {diag:?}"
    );
    assert!(
        diag.profile_match || diag.is_local_agent_index(),
        "diagnostic should be either active-local matched or expected local-agent profile: {diag:?}"
    );
}
...
async fn seed_rules(state: &McpState, items: &[(&str, &str)]) -> Vec<String> {
    ...
    for (i, (title, body)) in items.iter().enumerate() {
        ...
        let id = result["_meta"]["rule_id"]
            ...
            .expect("rule_id present")
            .to_owned();
        promote_rule_for_test(state, &id).await;
...
async fn search_rules_returns_index_not_full_body() {
    ...
    assert!(
        ...
        "file-scoped search_rules should record path-hint matches"
    );
}
...
fn rule_match_record_serializes_cloud_trust_proof() {
    let entry = crate::context::types::RuleMatchEvidenceRecord {
        ...
        cited_count: Some(2),
        trust_rate: Some(1.0),
        why: Some("path-hint; band 9/10; source cloud".to_owned()),
        evidence: Vec::new(),
    };
    ...
    assert_eq!(
        value["why"].as_str(),
        Some("path-hint; band 9/10; source cloud"),
...
#[tokio::test]
async fn pre_submit_compact_recall_caps_and_records_local_serve_proof() {
    let _repo_detection_guard = repo_detection_test_guard().await;
    let state = build_state().await;
    let repo_scopes = test_repo_scopes();
    ...
    for n in 1..=6 {
        let rule_id = remember_rule_with_patterns(
            &state,
            &format!("Pre-submit compact rule {n}"),
            &format!(
                "When reviewing pre-submit compact Rust changes, enforce rule {n} without expanding examples."
            ),
            &["src/**/*.rs"],
        )
        .await;
        ...
        sqlx::query(
            "UPDATE skills \
             SET origin = 'pr_review', \
                 description = description || ?2 \
             WHERE id = ?1",
        )
        .bind(&rule_id)
        .bind(format!(
            "\n\nSource evidence:\nSource: acme/widgets#{n}\nFile: src/lib.rs"
        ))
        .execute(&state.db)
        .await
        .expect("stamp source proof");
        rule_ids.push(rule_id);
    }

    let diff_files = vec!["src/lib.rs".to_owned()];
    let ctx = fetch_compact_relevant_rules_for_pre_submit_with_repo_scopes(
        &state.db,
        state.index_pool.as_ref().expect("index pool"),
        "src/lib.rs",
        &diff_files,
        "pre-submit compact Rust review",
        Some("hook-session"),
        &repo_scopes,
    )
    .await
    .expect("pre-submit recall");

    assert_eq!(ctx.rules_injected, 5);
    assert_eq!(ctx.rendered.lines().count(), 5);
    assert!(ctx.rendered.contains("← PR #"));
    assert!(!ctx.rendered.contains("without expanding examples"));
    assert!(
        ctx.rule_ids.iter().all(|id| rule_ids.contains(id)),
        "compact recall should return seeded rules, got {:?}",
        ctx.rule_ids
    );

    let rule_summary =
        crate::observability::mcp_rule_serves::summary_for_rule(&state.db, &ctx.rule_ids[0], 30)
            .await
            .expect("hook serve summary");
    let latest = rule_summary.latest.expect("latest hook serve evidence");
    assert_eq!(latest.tool, "hook_pre_submit");
    assert_eq!(latest.file_path.as_deref(), Some("src/lib.rs"));
    assert!(latest.estimated_tokens > 0);
}
...
async fn hook_injection_renders_why_segment_on_header() {
    ...
    assert!(
        ctx.rendered
            .contains("| why: path-hint; band 10/10; source conversation"),
...
async fn hook_post_edit_gate_keeps_on_subject_and_drops_adjacent_rule() {
    ...
    .await;
    scope_rule_to_test_repo(&state, &on_subject).await;
    ...
    .await;
    scope_rule_to_test_repo(&state, &adjacent).await;
...
async fn hook_intent_gate_applies_to_post_edit_but_not_bash_error_path() {
    ...
    let post_edit = fetch_relevant_rules_for_hook_with_repo_scopes(
        &state.db,
        index_pool,
        "binding/form.go",
        "post-edit\nreturn false instead of panic on invalid input",
    ...
    let bash_error = fetch_relevant_rules_for_bash_error_with_repo_scopes(
        &state.db,
        index_pool,
        "binding/form.go",
...
async fn search_rules_results_carry_compact_why_ranking() {
    ...
    assert_eq!(entry["sourceRank"], 4, "conversation source rank");
    assert_eq!(entry["sourceProvenance"]["origin"], "conversation");
    assert_eq!(entry["sourceProvenance"]["sourceRank"], 4);
    assert!(
        result["_meta"]["trace"]["candidateLimit"].is_number(),
        "_meta.trace must expose candidate accounting: {result}"
    );
    assert!(
        result["_meta"]["trace"]["dropReasons"].is_object(),
        "_meta.trace must expose drop reason counters: {result}"
    );
}
...
async fn hook_rebuild_failure_returns_error_instead_of_serving_stale_index() {
    ...
    let err = fetch_relevant_rules_for_hook_with_repo_scopes(
        &state.db,
        index_pool,
        "src/lib.rs",
...
async fn mcp_rejects_control_plane_mutation_tools() {
    ...
    let error = resp
        ...
        .unwrap_or_default();
    assert!(
        error.contains("intentionally not exposed through MCP") && error.contains("DiffLore CLI"),
        "control-plane mutation should be explicitly denied: {resp}"
    );
}

#[tokio::test]
async fn mcp_rejects_tools_not_in_explicit_allowlist() {
    let state = build_state().await;
    let req = call_tool(43, "cloud_login", json!({}));
    let resp = handle_message(&state, &req).await.unwrap();
    let error = resp
        .get("error")
        .and_then(|e| e.get("message"))
        ...
        .unwrap_or_default();
    assert!(
        error.contains("explicit allowlist"),
        "unapproved tools should fail closed at the MCP dispatch boundary: {resp}"
    );
}
...
async fn rule_timeline_orders_events_and_truncates_preview() {
    ...
    let rule_id = remember["_meta"]["rule_id"]
        ...
        .expect("rule_id in _meta")
        .to_owned();
    promote_rule_for_test(&state, &rule_id).await;
...
/// A cloud-managed semantic corpus served to a SHA1 fallback query embedder
...
async fn embedding_profile_mismatch_is_surfaced_not_silent() {
    ...
    // The test helper promotes the draft and builds the project index, which
    // persists the active embedder's profile into `rule_index_meta`.
    ...
    if sha1_fallback_active {
        ...
        // A cloud-managed semantic corpus queried by the 128d SHA1 fallback.
        overwrite_index_embedding_profile(index_pool, "cloud:managed").await;
        ...
        assert!(
            diag.degraded,
            "cloud semantic corpus + SHA1 query must be flagged degraded, got: {diag:?}"
        );
        assert!(
        ...
        assert!(
            !diag.profile_match,
            "sha1:local:128 must not match cloud:managed: {diag:?}"
        );
        assert!(
            matches!(diag.degraded_reason.as_deref(), Some("provider_fallback")),
