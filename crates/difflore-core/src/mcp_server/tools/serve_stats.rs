...
#[cfg(test)]
pub(crate) const MCP_RULE_WRITE_INDEX_TIMEOUT: std::time::Duration =
...
pub(crate) fn rerank_scored_rule_chunks_for_mcp_by_strict_file_matches(
    ...
    if !strict_skill_ids.is_empty() {
        ...
        reranked.sort_by(|a, b| {
            let a_band = crate::context::retrieval::relative_score_band(a.score, top_score);
            let b_band = crate::context::retrieval::relative_score_band(b.score, top_score);
            b_band.cmp(&a_band).then_with(|| {
                strict_skill_ids
                    .contains(&b.skill_id)
...
/// Cold-start helper shared by the hook and `search_rules`: fetch transferable
/// cross-repo starter rules. Matching `file_patterns` are path hints only; they
/// can boost ranking in retrieval but never hard-filter results.
///
...
pub(crate) async fn cross_repo_starter_scored(
    ...
    let Ok(candidates) = crate::context::retrieval::retrieve_rules_for_search(
        ...
        crate::context::retrieval::RuleSearchRetrievalOptions {
            ...
            // Latency-critical MCP path: query with the local lexical vector
            // only, never a provider round-trip in the tool-call path.
...
mod tests {
    ...
    #[test]
    fn mcp_path_hint_rerank_only_promotes_inside_same_score_band() {
        ...
        assert_eq!(
            ids(&reranked),
            vec!["universal", "strict-file"],
            "a low-relevance path hint must not outrank a much stronger rule"
        );

        let near_tie = rerank_scored_rule_chunks_for_mcp_by_strict_file_matches(
            vec![
                scored_rule(
                    "universal",
                    "Review: general handler guidance",
                ...
                scored_rule(
                    "leader",
                    "Review: strongest handler guidance",
                    "Preserve handler structure.",
                    1.0,
                ),
            ],
            "",
        ...
        assert_eq!(
            ids(&near_tie),
            vec!["leader", "strict-file", "universal"],
            "path hints may break ties inside the same score band"
        );
    }
    ...
    async fn cli_search_helper_and_mcp_helper_share_age_decay_ranking() {
        ...
        let cli_without_age = crate::context::retrieval::retrieve_rules_for_search(
            ...
            crate::context::retrieval::RuleSearchRetrievalOptions {
                ...
                embedding_timeout: Some(MCP_RULE_WRITE_INDEX_TIMEOUT),
                cold_start_retry: false,
