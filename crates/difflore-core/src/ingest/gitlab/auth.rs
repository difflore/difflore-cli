...
pub async fn load_stored_pat(host: &str) -> Option<String> {
    let pool = CloudClient::auth_pool_public().await.ok()?;
...
pub async fn configured_hosts() -> Vec<String> {
    let Ok(pool) = CloudClient::auth_pool_public().await else {
        return Vec::new();
    };
    let keys: Vec<String> =
        sqlx::query_scalar("SELECT key FROM auth WHERE key LIKE 'gitlab_pat::%' ORDER BY key")
            .fetch_all(&pool)
            .await
            .unwrap_or_default();
    keys.iter()
