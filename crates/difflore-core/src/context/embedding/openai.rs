...
impl Embedder for OpenAICompatEmbedder {
    ...
    async fn embed_batch(
        ...
        // Send the whole batch in ONE request — the OpenAI `/embeddings` API
        // accepts an array `input`. The trait default issues one HTTP call per
        // text, which is slow and makes a normal-sized corpus blow the bounded
        // index timeouts (recall/fix/MCP) and fall back to SHA1. Batching keeps
        // BYOK indexing on par with `CloudEmbedder`.
        let body = serde_json::json!({
            "model": self.model,
            "input": texts,
        });
