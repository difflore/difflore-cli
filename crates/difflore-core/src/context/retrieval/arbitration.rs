...
//!
//!   1. strict file-pattern hit (desc) — explicit file scope always leads;
//!   2. relative-score quantisation band, `floor((score / top_score) * 10)`
...
pub struct RuleRankingWhy {
    /// The rule's explicit `file_patterns` matched the target file.
    pub strict_hit: bool,
...
impl RuleRankingWhy {
    ...
    /// `strict-hit; band 9/10; source manual` (the `strict-hit;` prefix and
    /// the `source` segment are omitted when not applicable).
    #[must_use]
    pub fn compact(&self) -> String {
        let mut out = String::with_capacity(40);
        if self.strict_hit {
            out.push_str("strict-hit; ");
        }
...
fn compare_keys(
    ...
    // ① strict hit desc ② band desc ③ source rank asc ④ confidence desc
...
///
/// * `strict_skill_ids` — rules whose explicit `file_patterns` matched the
///   target scope (same set the strict rerank used).
...
mod tests {
    ...
    fn source_rank_follows_founder_ruling() {
        ...
        // Unknown origins (e.g. "pack") must never outrank ratified ones.
        assert_eq!(source_rank("pack"), UNKNOWN_SOURCE_RANK);
    ...
    #[test]
    fn strict_hit_outranks_band_and_source() {
        let mut scored = vec![
            chunk("manual-universal", 1.0, 0.9),
            chunk("conversation-strict", 0.2, 0.3),
        ];
        let origin_map = origins(&[
            ("manual-universal", "manual"),
            ("conversation-strict", "conversation"),
        ]);
        let strict: HashSet<String> = ["conversation-strict".to_owned()].into();
        let whys = arbitrate_rule_order(&mut scored, &strict, &origin_map, false);
        assert_eq!(
            ids(&scored),
            vec!["conversation-strict", "manual-universal"],
            "an explicit file-scope hit leads regardless of band or source"
        );
        assert!(whys["conversation-strict"].strict_hit);
    ...
    fn unknown_origin_ranks_below_every_known_source() {
        let mut scored = vec![
            chunk("pack-rule", 1.0, 0.9),
            chunk("conversation-rule", 1.0, 0.5),
        ];
        let origin_map = origins(&[("pack-rule", "pack"), ("conversation-rule", "conversation")]);
        arbitrate_rule_order(&mut scored, &HashSet::new(), &origin_map, false);
        assert_eq!(ids(&scored), vec!["conversation-rule", "pack-rule"]);
    }
    ...
    fn why_compact_renders_the_agreed_grammar() {
        ...
        assert_eq!(
            ...
            "strict-hit; band 9/10; source manual"
        );
