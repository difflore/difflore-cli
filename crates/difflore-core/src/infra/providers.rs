...
impl ProviderRow {
    ...
    fn parse_model_mapping(&self) -> std::collections::HashMap<String, String> {
        match serde_json::from_str(&self.model_mapping) {
            Ok(mapping) => mapping,
            Err(e) => {
                if crate::infra::env::debug_providers() {
                    eprintln!(
                        "[providers] failed to parse model_mapping for provider {}: {e}",
                        self.id
                    );
                }
    ...
    fn into_masked(self) -> ProviderRecord {
        ...
        let model_mapping = self.parse_model_mapping();
        ProviderRecord {
            id: self.id,
            name: self.name,
...
pub async fn set_active(db: &sqlx::SqlitePool, input: ProviderSetActiveInput) -> crate::Result<()> {
    ...
    if !input.is_active {
        let result = sqlx::query("UPDATE providers SET is_active = 0 WHERE id = ?1")
            .bind(&input.id)
            .execute(&mut *tx)
            .await?;
        if result.rows_affected() == 0 {
