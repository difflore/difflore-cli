...
/// v3: Codex dropped `Read` from its PostToolUse matcher — the handler no-ops
/// on Read, so matching it only cost a hook spawn per read for zero value.
/// v4: Windows hook commands route through the no-console launcher before
/// reaching `difflore-hook`, preventing GUI clients from flashing terminals.
/// v5: Windsurf stopped registering `post_mcp_tool_use`; MCP tool hooks map to
...
pub(super) enum BlockKind {
    ...
    McpJson,
    /// JSON lifecycle-hooks groups (Claude / Codex / Cursor / Gemini / Windsurf).
    HooksJson,
...
pub(super) enum ConfigFormat {
    ...
    /// Delegate to the agent's own CLI (Claude / Codex). Install dry-run
    /// output is rendered from `add_args` and the resolved binary so it matches
    /// the actual command, including the no-console launcher on Windows.
    CliDelegate {
        cli: &'static str,
...
/// surfaces Claude Code → Claude Code hooks → Codex → Codex hooks first, then
/// the rest.
pub(super) static AGENTS: &[AgentSpec] = &[
    ...
    AgentSpec {
        ...
        client: ClientId::Codex,
        scope: PathScope::Home,
        segments: &[".codex", "hooks.json"],
        display: "~/.codex/hooks.json",
        format: ConfigFormat::Hooks {
            surface: HookSurface::Codex,
        },
        detect: DetectSignal::ParentDirOrCli {
            cli: "codex",
            skip_reason: "`codex` CLI not on PATH and ~/.codex/ not found",
        },
        skip_if_plugin: false,
    },
    AgentSpec {
    ...
    AgentSpec {
        ...
        // opencode nests MCP servers under the top-level `mcp` key, and each
        // entry uses its own `{type:"local", command:[bin,"mcp-server"],
        // enabled:true}` shape — selected by `mcp_entry_shape_of` off the
        // OpenCode client, so the merge/uninstall/manifest machinery is reused.
        format: ConfigFormat::Json { servers_key: "mcp" },
        detect: DetectSignal::ParentDirOrCli {
            cli: "opencode",
            skip_reason: "~/.config/opencode/ not found and `opencode` CLI not on PATH",
        },
        skip_if_plugin: false,
    },
];
...
pub(super) fn install(
    ...
    match &spec.format {
        ...
        ConfigFormat::Json { servers_key } => match resolve_path(spec) {
            Ok(path) => finish_json_install(
                spec.name,
                &path,
                mcp_bin,
                servers_key,
                mcp_entry_shape_of(spec),
                dry_run,
            ),
        ...
        ConfigFormat::Hooks { surface } => match surface {
            ...
            HookSurface::Codex => install_codex_hooks(cli_bin, dry_run),
...
fn install_cli_delegate(
    ...
    if dry_run {
        return TargetOutcome {
            ...
            detail: cli_delegate_dry_run(cli, add_args, mcp_bin),
        };
    }
...
fn cli_delegate_dry_run(cli: &str, add_args: &[&str], mcp_bin: &str) -> String {
    let args = add_args
        .iter()
        .map(|arg| if *arg == "{bin}" { mcp_bin } else { *arg })
        .collect::<Vec<_>>()
        .join(" ");
    format!("would run: {cli} {args}")
}
...
pub(super) fn uninstall(spec: &AgentSpec, dry_run: bool) -> TargetOutcome {
    match &spec.format {
        ...
        ConfigFormat::Hooks { surface } => match surface {
            ...
            HookSurface::Codex => uninstall_codex_hooks(dry_run),
    ...
    match std::process::Command::new(cli).args(remove_args).output() {
        Ok(o) if o.status.success() => TargetOutcome {
            ...
            detail: installed_detail
                .replace(" via `codex mcp add`", &format!(" via `{cli} mcp remove`"))
                .replace(" via `claude mcp add`", &format!(" via `{cli} mcp remove`")),
        },
...
pub(super) fn canonical_target_key(name: &str) -> String {
    ...
    match lower.as_str() {
        ...
        "codex hooks" | "codex code hooks" => return "codex hooks".into(),
        _ => {}
    }
...
mod tests {
    ...
    #[test]
    fn cli_delegate_dry_run_renders_resolved_binary() {
        assert_eq!(
            cli_delegate_dry_run(
                "codex",
                &["mcp", "add", "difflore", "--", "{bin}", "mcp-server"],
                r"C:\Users\me\.cargo\bin\difflore-launcher.exe",
            ),
            r"would run: codex mcp add difflore -- C:\Users\me\.cargo\bin\difflore-launcher.exe mcp-server",
        );
    }
}
