...
pub(super) fn resolve_difflore_binary() -> Result<String, String> {
    ...
    Ok(s.strip_prefix(r"\\?\").map(ToOwned::to_owned).unwrap_or(s))
}
...
pub(super) fn probe_runtime_mcp_server(bin: &str) -> McpRuntimeProbe {
    ...
    if let Some(mut stdin) = child.stdin.take() {
        if let Err(e) = stdin.write_all(probe_input.as_bytes()) {
            ...
            return McpRuntimeProbe {
                state: RuntimeProbeState::Failed,
                detail: format!("could not write JSON-RPC probe to MCP stdin: {e}"),
                initialized: false,
                tools_listed: false,
                tool_call_completed: false,
                tool_call_name: None,
        ...
        return McpRuntimeProbe {
            state: RuntimeProbeState::Failed,
            detail: "MCP child did not expose stdin".into(),
            initialized: false,
            tools_listed: false,
            tool_call_completed: false,
            tool_call_name: None,
    ...
    loop {
        match child.try_wait() {
            ...
            Ok(None) => {
                ...
                return McpRuntimeProbe {
                    state: RuntimeProbeState::Timeout,
                    detail,
                    initialized: false,
                    tools_listed: false,
                    tool_call_completed: false,
            ...
            Err(e) => {
                ...
                return McpRuntimeProbe {
                    state: RuntimeProbeState::Failed,
                    detail: format!("could not wait for MCP self-check: {e}"),
                    initialized: false,
                    tools_listed: false,
                    tool_call_completed: false,
                    tool_call_name: None,
    ...
    match child.wait_with_output() {
        ...
        Err(e) => McpRuntimeProbe {
            state: RuntimeProbeState::Failed,
            detail: format!("could not collect MCP self-check output: {e}"),
            initialized: false,
            tools_listed: false,
            tool_call_completed: false,
            tool_call_name: None,
