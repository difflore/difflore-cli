...
pub enum HookEvent {
    ...
    PostToolUse {
        tool_name: String,
        /// Client-reported working directory for the hook event. Important for
        /// agents that can edit files outside the process cwd.
        #[serde(default, skip_serializing_if = "Option::is_none")]
        cwd: Option<String>,
        #[serde(default, skip_serializing_if = "Option::is_none")]
        file_path: Option<String>,
        /// All files touched by the tool call when the client exposes them
        /// (for example Codex `apply_patch` with multiple file hunks).
...
pub struct HookResult {
    ...
    /// Structured reason a hook fire produced no injected context. Local audit
    /// field only; skipped on the wire and persisted by the hook fire log.
    #[serde(default, skip)]
...
impl HookResult {
    ...
    pub(crate) const fn noop_with_reason(reason: InjectionDropReason) -> Self {
        Self {
            continue_: true,
            system_message: None,
            additional_context: None,
    ...
    pub(crate) fn target_file_path(&self) -> Option<String> {
        match self {
            ...
            Self::PostToolUse {
                ...
                target_files,
                ..
            } => file_path.clone().or_else(|| target_files.first().cloned()),
            _ => None,
        }
    }
}
