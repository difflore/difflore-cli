...
struct HookFireEntry {
    ...
    /// Structured local-only reason this fire produced no injected context.
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
pub(crate) struct HookFireSummary {
    ...
    /// No-injection fires by structured reason.
    pub(crate) dropped_by_reason: std::collections::BTreeMap<String, usize>,
...
fn remember_hook_fire_full(
    ...
    log.entries.push(HookFireEntry {
        ...
        elapsed_ms,
        drop_reason: if rules_injected.unwrap_or(0) == 0 {
            drop_reason
        } else {
            None
        },
    ...
    if let Ok(json) = serde_json::to_string_pretty(&log) {
        // Atomic write: hook fires can run concurrently (deferred logging spawns
        // a detached thread), so a plain `fs::write` could be torn / truncated.
...
pub(crate) fn hook_fire_summary_24h() -> HookFireSummary {
        ...
        .filter(|entry| entry.ts_ms >= cutoff)
    {
        ...
        *summary.by_event.entry(entry.event).or_insert(0) += 1;
        ...
        if entry.rules_injected.unwrap_or(0) == 0
            && let Some(reason) = entry.drop_reason
        {
            *summary
                .dropped_by_reason
