...
static STDERR_COLOR_CACHE: OnceLock<ColorSupport> = OnceLock::new();

// Color gate for the stderr surface (errors + spinner). Cached like the stdout
// gate; consults `stderr().is_terminal()` so the channel each surface writes to
// drives the decision.
fn stderr_color_support() -> ColorSupport {
    *STDERR_COLOR_CACHE.get_or_init(|| detect_color_support_for_tty(stderr().is_terminal()))
}

// Render a colored stderr line, dropping ANSI when the stderr gate says the
// stream isn't a color-capable terminal (redirected to a file, NO_COLOR, etc.).
// The color helpers paint based on the stdout gate, so this re-strips for stderr.
fn for_stderr(rendered: String) -> String {
    if matches!(stderr_color_support(), ColorSupport::None) {
        strip_ansi(&rendered)
    } else {
        rendered
    }
...
static TERMINAL_WIDTH_CACHE: OnceLock<usize> = OnceLock::new();

pub(crate) fn terminal_width() -> usize {
    *TERMINAL_WIDTH_CACHE.get_or_init(terminal_width_uncached)
}

fn terminal_width_uncached() -> usize {
    use difflore_core::infra::env;
    // Prefer the real controlling terminal's width (Windows console API /
    // TIOCGWINSZ via stdout); $COLUMNS isn't exported by cmd.exe/PowerShell and
    // is only a shell variable on Unix. Fall back to $COLUMNS, then the default.
    terminal_size::terminal_size_of(stdout())
        .map(|(terminal_size::Width(cols), _)| usize::from(cols))
        .or_else(|| env::var(env::COLUMNS).and_then(|raw| raw.trim().parse::<usize>().ok()))
...
/// Word-wrap `text` so that, when printed after a `column` cells wide prefix,
/// no line exceeds the terminal width, folding continuation lines under the
/// same column (i.e. indented by `column` spaces) instead of at column 0.
///
/// Returns the wrapped body with continuation lines already indented; the
/// caller still emits the first-line prefix itself. East-Asian width aware.
pub(crate) fn wrap_after_column(text: &str, column: usize) -> String {
    let width = terminal_width();
    let avail = width.saturating_sub(column).max(MIN_WRAP_WIDTH);
    if visible_width_ansi(text) <= avail {
    ...
    for word in words {
        ...
        if wrote_word_on_line && line_width + space_width + word_width > avail {
            out.push('\n');
            out.push_str(&indent);
            line_width = 0;
...
fn detect_color_support_uncached() -> ColorSupport {
    detect_color_support_for_tty(stdout().is_terminal())
}

// Same NO_COLOR + terminal-class logic as the stdout gate, but parameterized on
// the TTY-ness of whichever stream the caller actually writes to. Surfaces that
// emit to stderr (errors, spinner) must consult this with `stderr().is_terminal()`
// so `cmd 2> err.log` doesn't get raw ANSI and `cmd | less` keeps a colored
// interactive error/spinner channel.
fn detect_color_support_for_tty(is_tty: bool) -> ColorSupport {
    use difflore_core::infra::env;
...
impl Spinner {
    ...
    pub(crate) fn finish_ok(mut self, msg: &str) {
        self.stop_and_clear();
        eprintln!("{}", for_stderr(format!("{} {}", emerald(sym::OK), msg)));
    }

    /// Final line: `X <msg>` (danger), spinner cleared.
    pub(crate) fn finish_err(mut self, msg: &str) {
        self.stop_and_clear();
        eprintln!("{}", for_stderr(format!("{} {}", danger(sym::ERR), msg)));
    }
...
fn spinner_line_width(label: &str) -> usize {
    use unicode_width::UnicodeWidthStr;
    UnicodeWidthStr::width(SPIN_FRAMES[0]) + 1 + UnicodeWidthStr::width(label) + 2
}
...
mod tests {
    ...
    fn wrap_human_text_keeps_bullet_continuation_indented() {
        let wrapped = wrap_human_text_for_width(
            "  - semantic recall: local keyword fallback (managed: difflore cloud login; BYOK: difflore embeddings setup)",
            56,
        );
