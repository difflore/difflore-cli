...
pub(in crate::commands::status) async fn build_value_loop_evidence_for_candidate(
    ...
    let mcp_summary =
        difflore_core::observability::mcp_rule_serves::summary_for_rule(db, &candidate.rule_id, 30)
            .await
            .ok()?;
...
async fn causal_top3_recall_for(
    ...
    let recall =
        sqlx::query_as::<_, difflore_core::observability::rule_outcomes::TopRecallEvidence>(
            r"SELECT rule_id,
                  repo_full_name,
                  file_path,
